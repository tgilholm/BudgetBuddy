...
public abstract class javadb extends RoomDatabase
{
    ...
    private static volatile javadb DB_INSTANCE;
     ...
     */
    public static javadb getDBInstance(final Context context)
    {
        // First check
        if (DB_INSTANCE == null)
        {
            synchronized (javadb.class)
            {    // Sync and check again
                if (DB_INSTANCE == null)
                {
                    ...
                    DB_INSTANCE = Room.databaseBuilder(context.getApplicationContext(), javadb.class, "budgetbuddy_DB"  // Name of the DB
